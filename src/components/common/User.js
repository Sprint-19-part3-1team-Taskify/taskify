 ...
 *   - 사용자 프로필 이미지 URL
 *   - 이미지가 있으면 이미지를 표시하고, 없으면 첫 글자를 표시합니다
 *
 ...
 * <User value="홍길동" />
 * // 프로필 이미지와 함께
 * <User value="홍길동" profileImageUrl="https://example.com/image.jpg" />
 * // 아바타만 표시
...
export default function User({ value, profileImageUrl, type, hiddenName, remain }) {
  if (!value) return null;

  const [imageError, setImageError] = useState(false);
  const first = value.slice(0, 1);
  const colorIndex = value.charCodeAt(0) % colors.length;
  const hasImage = profileImageUrl && profileImageUrl.trim() !== '' && !imageError;

  return (
    <div className={`${styles.user} ${styles[type]}`}>
      <span
        className={`${styles.first} ${remain ? styles['remain'] : ''} ${hasImage ? styles.hasImage : ''}`}
        style={{ backgroundColor: hasImage ? 'transparent' : colors[colorIndex] }}
      >
        {hasImage ? (
          <Image
            src={profileImageUrl}
